...
class Slider_ctrl
{
    ...
    public function list($req = null)
    {
        ...
        $total_page = $this->slider_list(ord: "DESC", limit: 10000, active: 1);
        $tp = count($total_page);
        if ($tp %  $data_limit == 0) {
            $tp = $tp / $data_limit;
        } else {
        ...
        if (isset($req->search)) {
            $slider_list = $this->slider_search_list($keyword = $req->search, $ord = "DESC", $limit = $page_limit, $active = 1);
        } else {
            $slider_list = $this->slider_list(ord: "DESC", limit: $page_limit, active: 1);
        }
        $context = (object) array(
            'page' => 'sliders/list.php',
            'data' => (object) array(
                'req' => obj($req),
    ...
    public function trash_list($req = null)
    {
        ...
        $total_page = $this->slider_list(ord: "DESC", limit: 10000, active: 0);
        $tp = count($total_page);
        if ($tp %  $data_limit == 0) {
            $tp = $tp / $data_limit;
        } else {
        ...
        if (isset($req->search)) {
            $slider_list = $this->slider_search_list($keyword = $req->search, $ord = "DESC", $limit = $page_limit, $active = 0);
        } else {
            $slider_list = $this->slider_list(ord: "DESC", limit: $page_limit, active: 0);
        }
        $context = (object) array(
            'page' => 'sliders/list.php',
            'data' => (object) array(
    ...
    public function edit($req = null)
    {
        $req = obj($req);
        $context = (object) array(
            'page' => 'sliders/edit.php',
            'data' => (object) array(
                'req' => obj($req),
                'slider_detail' => $this->slider_detail($req->id),
    ...
    public function save($req = null)
    {
        ...
        if (isset($request->title)) {
            ...
            if (intval($postid)) {
                ...
                echo js_alert('Slider created');
                echo go_to("/admin/slider/list");
            } else {
                echo js_alert('Slider not created');
                return false;
            }
        }
    }
    public function toggle_trending($req = null)
    {
        ...
        if (isset($request->content_id) && isset($request->action) && ($request->action == 'is_trending' || $request->action == 'is_featured')) {
            ...
            if (count($trending_post) > 0) {
                ...
                $res['msg'] = 'success';
                $res['data'] = "Page marked as $request->action";
            }
    ...
    public function update($req = null)
    {
        ...
        if (isset($request->title)) {
            ...
            try {
                ...
                echo go_to(route('sliderEdit', ['id' => $request->id]));
                exit;
    ...
    public function restore($req = null)
    {
        ...
        try {
            ...
            echo go_to(route('sliderTrashList'));
            exit;
        } catch (PDOException $e) {
    ...
    // Post list
    public function slider_list($ord = "DESC", $limit = 5, $active = 1, $sort_by = 'views')
    {
        $cntobj = new Model('content');
        return $cntobj->filter_index(array('content_group' => 'slider', 'is_active' => $active), $ord, $limit, $change_order_by_col=$sort_by);
    }
    public function slider_search_list($keyword, $ord = "DESC", $limit = 5, $active = 1)
    {
        $cntobj = new Model('content');
        ...
        return $cntobj->search(
            ...
            ord: $ord,
            limit: $limit,
            whr_arr: array('content_group' => 'slider', 'is_active' => $active)
        );
    }

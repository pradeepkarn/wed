...
<section>
  <div class="container">
    <div class="row">
      ...
      <div class="col-md-9" data-aos="fade-up">
        <h3 class="category-title">Category: <?php echo $cat ? $cat->title : null; ?></h3>
        <div id="blog-posts">
          <?php
          /*
  ...
  <script>
    ...
    // Function to load the next page of blog posts
    function loadMorePosts() {
      ...
      fetch(`<?php echo "/" . home . route('catOnScroll', ['slug' => $catslug]); ?>?page=${currentPage}&limit=${postsPerPage}`)
        .then(response => response.json())
        .then(posts => {
          // Append the new posts to the container
          const container = document.getElementById('blog-posts');
          posts.forEach(post => {
            // Check if the post has already been loaded
            if (!loadedPostIds.includes(post.id)) {
              const postElement = createPostElement(post);
              container.appendChild(postElement);

              // Add the post ID to the loadedPostIds array
    ...
    function createPostElement(post) {
      ...
      element.classList.add('d-md-flex', 'post-entry-2', 'half');
      element.innerHTML = `
    <a href="${post.link}" class="me-4 thumbnail">
      <img style="height:200px; width:100%; object-fit:cover;" src="${post.banner}" alt="${post.title}" class="img-fluid">
    </a>
    <div>
      <div class="post-meta"><span class="date">${post.category_name}</span> <span class="mx-1">&bullet;</span> <span>${post.date}</span></div>
      <h3><a href="${post.link}">${post.title}</a></h3>
      <p>${post.content}</p>
      <div class="d-flex align-items-center author">
        <div class="photo"><img src="${post.author_image}" alt="" class="img-fluid"></div>
        <div class="name">
  ...
  `;
    ...
    // Event listener to load more posts when the user scrolls to the bottom of the page

...
class Page_ctrl
{
    ...
    public function list($req = null)
    {
        ...
        $total_page = $this->page_list(ord: "DESC", limit: 10000, active: 1);
        $tp = count($total_page);
        if ($tp %  $data_limit == 0) {
            $tp = $tp / $data_limit;
        } else {
        ...
        if (isset($req->search)) {
            $page_list = $this->page_search_list($keyword = $req->search, $ord = "DESC", $limit = $page_limit, $active = 1);
        } else {
            $page_list = $this->page_list(ord: "DESC", limit: $page_limit, active: 1);
        }
        $context = (object) array(
            'page' => 'pages/list.php',
            'data' => (object) array(
    ...
    public function trash_list($req = null)
    {
        ...
        $total_page = $this->page_list(ord: "DESC", limit: 10000, active: 0);
        $tp = count($total_page);
        if ($tp %  $data_limit == 0) {
            $tp = $tp / $data_limit;
        } else {
        ...
        if (isset($req->search)) {
            $page_list = $this->page_search_list($keyword = $req->search, $ord = "DESC", $limit = $page_limit, $active = 0);
        } else {
            $page_list = $this->page_list(ord: "DESC", limit: $page_limit, active: 0);
        }
        $context = (object) array(
            'page' => 'pages/list.php',
            'data' => (object) array(
    ...
    public function edit($req = null)
    {
        ...
        $context = (object) array(
            ...
            'data' => (object) array(
                'req' => obj($req),
                'page_detail' => $this->page_detail($req->id),
    ...
    public function save($req = null)
    {
        ...
        if (isset($request->title)) {
            ...
            if (intval($postid)) {
                ...
                echo js_alert('Page created');
                echo go_to("/admin/page/list");
            } else {
